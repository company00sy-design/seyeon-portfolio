...
function parseImages(value){
  ...
  if(typeof value==='string'){
    try{const parsed=JSON.parse(value);return Array.isArray(parsed)?parsed.filter(Boolean):value?[value]:[];}catch{return value?[value]:[];}
  }
  return [];
}
...
function imageCard(w,extra=''){
  const c=w.category;
  if(c==='detail'){
    const thumb=w.image||w.detail_images[0]||'';
    return `<article class="work-card ${extra}" data-id="${esc(w.id)}" data-category="detail"><div class="detail-preview"><div class="detail-preview-inner">${thumb?`<img src="${esc(thumb)}" alt="${esc(w.title)}" loading="lazy">`:'<div class="media-placeholder">DETAIL PAGE</div>'}<span class="detail-preview-label">VIEW FULL PAGE ↗</span></div></div><div class="work-meta"><span>DETAIL PAGE</span><h3>${esc(w.title)}</h3></div></article>`;
  }
  if(c==='social'||c==='video'){
    const src=w.image;
  ...
  if(c==='web'){
    return `<article class="work-card ${extra}" data-id="${esc(w.id)}" data-category="web"><div class="web-preview"><div class="browser-bar"><i></i><i></i><i></i></div>${w.image?`<img src="${esc(w.image)}" alt="${esc(w.title)}" loading="lazy">`:'<div class="media-placeholder">WEB</div>'}<span class="web-preview-label">VIEW SITE ↗</span></div><div class="work-meta"><span>WEB</span><h3>${esc(w.title)}</h3></div></article>`;
  }
